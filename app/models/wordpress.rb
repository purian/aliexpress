...
class Wordpress < ActiveRecord::Base
  ...
  def get_orders
    ...
    all_orders = woocommerce.get("orders?filter[limit]=500&filter[order]=desc&status=on-hold&fields=id,shipping_address,billing_address,line_items").parsed_response
