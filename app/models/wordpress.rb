...
class Wordpress < ActiveRecord::Base
  ...
  def get_orders
    ...
    all_orders = woocommerce.get("orders?filter[limit]=200&filter[order]=asc&status=processing&fields=id,completed_at,shipping_address,billing_address,line_items").parsed_response
